...
class Fern2 : public Serializable
{
    protected:
    private:
    public:
        vector<FourBitBP*> node;
        ...
        Fern2(const Fern2& other)
        {
            this->node.resize(other.node.size(),0);
            for(int i = 0; i < this->node.size(); i++)
            {
                this->node[i] = new FourBitBP(*other.node[i]);
            }
        ...
        Fern2(int nNodes, float minScale, float maxScale);
        virtual ~Fern2();
        void release();


        int leafIndex(unsigned char c1, unsigned char c2);

        void train(unsigned char c1, unsigned char c2, int patchLabel);
        ...
        virtual void restore(istream& in)
        {
            ...
            {
                node.resize(nNodes, 0);
                for(int i = 0; i < nNodes; i++) this->node[i] = new FourBitBP();
            }
        ...
        friend ostream& operator << (ostream& out, const Fern2& obj)
        {
            obj.save(out);
        ...
        friend istream& operator >> (istream& in, Fern2& obj)
        {
            obj.restore(in);
